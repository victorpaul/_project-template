...
import org.pac4j.core.context.{JEEContext, WebContext}
import org.pac4j.core.profile.{CommonProfile, ProfileManager}

abstract class SecurityController {

  def getSignedInProfile(implicit request: HttpServletRequest, response: HttpServletResponse): LoggedInDemoUser = {
    val webContext = new JEEContext(request, response)
    val manager = new ProfileManager[CommonProfile](webContext)
    manager.get(false).get.getAttribute("user", classOf[LoggedInDemoUser])
  }
}
